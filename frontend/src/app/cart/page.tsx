...
import { useClearCartMutation, useGetCartItemsQuery, useRemoveCartItemMutation, useUpdateCartItemMutation } from '@/store/api/splits/cart'
...
const CartPage: React.FC = () => {
  ...
  const handleQuantityChange = async (id: number, newQuantity: number) => {
    try {
      if (newQuantity < 1) {
        ...
        await updateItem({ id, data: { quantity: newQuantity } }).unwrap()
      }
    } catch (err) {
  ...
  return (
    <AuthGuard requireAuth requireCustomer>
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {items.map((item) => (
                <div key={item.id} className="px-6 py-4 flex items-center space-x-4">
                  <img
                    src={item.product.image_url || 'https://images.pexels.com/photos/230544/pexels-photo-230544.jpeg'}
                    alt={item.product.name}
                    className="w-16 h-16 object-cover rounded-lg"
                  />
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white truncate">{item.product.name}</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{item.product.brand}</p>
                    <p className="text-lg font-bold text-blue-600 dark:text-blue-400">${item.product.price.toFixed(2)}</p>
                  </div>
